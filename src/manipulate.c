...
int	manipulate(t_stack	*s, int cmd)
{
	...
	if (cmd == sa)
	{
		...
		s->a[s->a_len - 2] = i;
		if (tests(s) || write(STDOUT_FILENO, "sa\n", 3) < 0)
			return (1);
	}
	else if (cmd == sb)
	{
		...
		s->b[s->b_len - 2] = i;
		if (tests(s) || write(STDOUT_FILENO, "sb\n", 3) < 0)
			return (1);
	}
	else
	{
...
int	manipulate1(t_stack	*s, int cmd)
{
	...
	if (cmd == ss)
	{
		...
		s->b[s->b_len - 1] = s->b[s->b_len - 2];
		s->b[s->b_len - 2] = i;
		if (tests(s) || write(STDOUT_FILENO, "ss\n", 3) < 0)
			return (1);
	}
	else if (cmd == pa)
	{
		pop_push(s->b, &s->b_len, s->a, &s->a_len);
		if (tests(s) || write(STDOUT_FILENO, "pa\n", 3) < 0)
			return (1);
	}
	else
	{
...
int	manipulate2(t_stack	*s, int cmd)
{
	if (cmd == pb)
	{
		...
		if (tests(s) || write(STDOUT_FILENO, "pb\n", 3) < 0)
			return (1);
	}
	else if (cmd == ra)
	{
		pop_push(s->a, &s->a_len, s->a_back, &s->a_back_len);
		if (tests(s) || write(STDOUT_FILENO, "ra\n", 3) < 0)
			return (1);
	}
	else if (cmd == rb)
	{
		pop_push(s->b, &s->b_len, s->b_back, &s->b_back_len);
		if (tests(s) || write(STDOUT_FILENO, "rb\n", 3) < 0)
			return (1);
	}
	else
	{
...
int	manipulate3(t_stack	*s, int cmd)
{
	if (cmd == rr)
	{
		pop_push(s->a, &s->a_len, s->a_back, &s->a_back_len);
		pop_push(s->b, &s->b_len, s->b_back, &s->b_back_len);
		if (tests(s) || write(STDOUT_FILENO, "rr\n", 3) < 0)
			return (1);
	}
	else if (cmd == rra)
	{
		pop_push(s->a_back, &s->a_back_len, s->a, &s->a_len);
		if (tests(s) || write(STDOUT_FILENO, "rra\n", 4) < 0)
			return (1);
	}
	else if (cmd == rrb)
	{
		pop_push(s->b_back, &s->b_back_len, s->b, &s->b_len);
		if (tests(s) || write(STDOUT_FILENO, "rrb\n", 4) < 0)
			return (1);
	}
	else
	{
...
int	manipulate4(t_stack	*s, int cmd)
{
	if (cmd == rrr)
	{
		pop_push(s->a_back, &s->a_back_len, s->a, &s->a_len);
		pop_push(s->b_back, &s->b_back_len, s->b, &s->b_len);
		if (tests(s) || write(STDOUT_FILENO, "rrr\n", 4) < 0)
			return (1);
	}
	else
		return (1);
